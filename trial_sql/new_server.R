...
server <- function(input, output, session) {
  ...
  new_data <- eventReactive(input$save, {
    new_data <- list()
    new_data[['inclusion']] <- list()
    new_data[['exclusion']] <- list()
    if (!is.null(input$getData)){
      inclu <- sample()[['inclusion']]
      exclu <- sample()[['exclusion']]
      ...
      for (i in c(1:n_i)){
        ...
        if (length(temp_lst) != 0){
          for (j in c(1:length(temp_lst))){
            template_id <- temp_lst[[j]]
            idx_temp <- paste(idx_temp0, j)
            new_data[['inclusion']][[i]][["mapped_templates"]][[j]] <- list()
            new_data[['inclusion']][[i]][["mapped_templates"]][[j]][['template']] <- template_id
            
            ## Condition --------
            if (template_id == 'Diagnosis'){
              new_data[['inclusion']][[i]][["mapped_templates"]][[j]][['template']] <- 'Condition by Diagnosis Code'
              ...
              ## Event --------  
            } else if (template_id == 'Event 1'|template_id == 'Event 2'|template_id == 'Event 3'){
              new_data[['inclusion']][[i]][["mapped_templates"]][[j]][['template']] <- 'Event'
              ...
              ## Lab -------- 
            } else if (template_id == 'Lab 1' | template_id == 'Lab 2' | template_id == 'Lab 3'){
              new_data[['inclusion']][[i]][["mapped_templates"]][[j]][['template']] <- 'Lab' 
      ...
      for (i in c(1:length(exclu))){
        idx <- n_i + i
        new_data[['exclusion']][[i]] <- list()
        idx_temp0 <- paste('exclu', i)
        cri_id <- paste0('temp_page_', idx)
        ...
        if (length(temp_lst) != 0){
          for (j in c(1:length(temp_lst))){
            template_id <- temp_lst[[j]]
            idx_temp <- paste(idx_temp0, j)
            new_data[['exclusion']][[i]][["mapped_templates"]][[j]] <- list()
            new_data[['exclusion']][[i]][["mapped_templates"]][[j]][['template']] <- template_id
            ...
            if (template_id == 'Diagnosis'){
              ...
              new_data[['exclusion']][[i]][["mapped_templates"]][[j]][['template']] <- 'Event'
              ...
              new_data[['exclusion']][[i]][["mapped_templates"]][[j]][['template']] <- 'Lab'
    ...
    if (input$addCriNum != '' & input$addCriNum != '0'){
      ...
      n_inclu <- length(new_data[['inclusion']])
      n_exclu <- length(new_data[['exclusion']])
